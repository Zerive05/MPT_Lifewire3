<div>
    <div class="container mt-2">
        <div class="card">
            ...
            @if ($penjualan_selected_id)
                <div class="m-3">
                    <a wire:click="delete_confirmation('')" class="btn btn-danger btn-sm mb-3" data-bs-toggle="modal"
                        data-bs-target="#modalhapus">Del {{ count($penjualan_selected_id) }} Data</a>
                </div>
            @endif
            <div class="mx-3">
                    ...
                    <div class="modal-content">
                        ...
                        <form>
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    <label for="penjualan1" class="form-label">Jumlah</label>
                                    <input type="number" class="form-control" id="penjualan`"
                                        placeholder="Jumlah penjualan" wire:model="jumlah">
                                </div>
                                <div class="mb-3">
                                    <label for="penjualan1" class="form-label">Tanggal</label>
                                    <input type="date" class="form-control" id="penjualan`"
                                        placeholder="Tanggal penjualan" wire:model="tanggal">
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" wire:click="clear()"
                ...
                <table class="table table-bordered table-sortable">
                    ...
                    <tbody>
                        @foreach ($datapenjualan as $key => $value)
                            <tr>
                                <td>
                                    <input type="checkbox" wire:model.live="penjualan_selected_id"
                                        wire:key="{{ $value->id }}" value="{{ $value->id }}">
                                </td>
                                <td>{{ $datapenjualan->firstitem() + $key }}</td>
